...
public class RecordWithKeyInCollectionExistsValidator  implements ConstraintValidator<RecordWithKeyExists, Collection<?>> {
	//Implementation Note:
	...
	private RecordWithKeyExists annotation;
	...
	public boolean isValid(Collection<?> keys, ConstraintValidatorContext context) {
		if (keys == null || keys.isEmpty()) {
			...
			recordWithKeyExistsValidator.initialize(annotation);
			for(Object o: keys){
				if(!(o instanceof Key)){
					throw new IllegalArgumentException(String.format("Expected type %s, but got %s", Key.class.getName(), o.getClass() ));
				}
				Key<?> k = (Key<?>)o;
				allValid = recordWithKeyExistsValidator.isValid(k, context);
				if (!allValid) {
					break;
				}
			}
			if(!allValid){
		        ...
		        context.buildConstraintViolationWithTemplate(RecordWithKeyExists.MESSAGE_WHEN_TARGET_IS_COLLECTION).addConstraintViolation();
			}
			return allValid;
		}
	}
